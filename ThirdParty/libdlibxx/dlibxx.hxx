...
auto fptr_cast(void *fptr) -> std::function<Ret(Args...)>
{
    using function_type = Ret (*)(Args...);
    using ptr_size_type =
        std::conditional<sizeof(fptr) == 4, long, long long>::type;
...
public:
    ...
    inline boost::optional<T> create(char const *fname, Args &&...args) const
    {
        auto factory_symbol = this->symbol_lookup_impl<T, Args...>(fname);
        if (factory_symbol)
            return factory_symbol.get()(std::forward<Args>(args)...);
        else
            return boost::optional<T>();
    }

    template <typename T, typename... Args>
    inline boost::optional<T> create(std::string const &fname, Args &&...args) const
    {
    ...
    inline bool loaded() const { return handle_ != nullptr; }

private:
    // Member data.
    std::string name_ = "";
    ::dlibxx::resolve resolve_time_ = ::dlibxx::resolve::lazy;
        ...
        -> boost::optional<std::function<Ret(Args...)>>
    {
        ...
        auto fptr =
            ::dlibxx::util::fptr_cast<Ret, Args...>(::dlsym(handle_, fname));
